...
const buildPlainEnglishSummaryReport = ({ data, fileId, anomalies, suspectIds, componentHealth }: { data: AnalysisResult; fileId?: string; anomalies: number; suspectIds: number; componentHealth: number }) => {
  ...
  const sections = [
    ...
    ["Systems Active", [`Active module groups: ${[systems.some((row) => String(row.category ?? "").includes("body")) ? "body/chassis" : "", systems.some((row) => String(row.category ?? "").includes("powertrain")) ? "powertrain" : "", diagnosticPresent ? "diagnostic" : ""].filter(Boolean).join(", ") || "general CAN housekeeping"}.`, hasMotionSignals ? "The network contains some dynamic candidates, so it may include operating-state traffic." : "The traffic looks normal for an ignition-on or idle vehicle: modules are awake and communicating, but movement-related frames are absent.", noisyIds.length ? `IDs that appear unusually noisy or dominant: ${noisyIds.join(", ")}. These should be checked first for chatter, repeated status messages, or a module talking more than expected.` : "No module appears overwhelmingly noisy; traffic distribution looks balanced for this capture.", quietIds.length ? `Very quiet IDs: ${quietIds.join(", ")}. These may be one-time status frames, sleeping modules, or messages that only appear during specific actions.` : "No unusually quiet ID group stands out from the available deep-dive data."]],
    ...
    ["Reverse-Engineering Insights", [volatileIds.length ? `Best DBC starting points: ${volatileIds.join(", ")}. These IDs have changing payloads or volatile bytes, making them better candidates for live signals.` : "There are limited high-variation IDs, so this capture is better for mapping housekeeping/status traffic than extracting driving signals.", statusIds.length ? `Likely status, counter, or housekeeping frames: ${statusIds.join(", ")}. These IDs are stable or low-change and may represent module state, keep-alive traffic, counters, or simple flags.` : "No clear housekeeping/status group was isolated from the available deep-dive rows.", `${activeBits} active bit candidate(s) and ${highEntropyBytes} changing byte area(s) are the main places to inspect first in future DBC work.`, !speedCandidates.length && !rpmCandidates.length && !pedalCandidates.length ? "No speed/RPM/pedal candidates were found because the capture does not contain the repeated, changing, motion-shaped patterns those signals usually produce during driving." : "Validate candidate IDs by recording controlled actions and confirming that bytes rise, fall, or toggle with the physical vehicle behavior."]],
    ["What This Log Does Not Contain", [speedCandidates.length ? "A possible speed signal is present, but it still needs validation." : "No speed signal.", rpmCandidates.length ? "A possible RPM signal is present, but it still needs validation." : "No RPM signal.", pedalCandidates.length ? "A possible pedal/brake signal is present, but it still needs validation." : "No pedal or brake signal.", "No clear steering or wheel-speed activity was identified.", diagnosticPresent ? "Diagnostic-shaped traffic appears present." : "No diagnostic request/response traffic was detected.", timelineRows.length ? "Some anomaly events are available for review." : "No event timeline changes were detected."]],
