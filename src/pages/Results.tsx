...
const buildPlainEnglishSummaryReport = ({ data, fileId, anomalies, suspectIds, componentHealth }: { data: AnalysisResult; fileId?: string; anomalies: number; suspectIds: number; componentHealth: number }) => {
  ...
  const sections = [
    ...
    ["What This Log Is", ["This is a housekeeping/idle-state capture, not a road-test capture.", "Modules are awake and broadcasting periodic status frames.", "The log is useful for module mapping, baseline behavior, status frames, counters, timers, flags, and identifying which IDs are stable versus dynamic.", "The log is not useful for decoding motion-related signals because it does not contain the physical driving actions needed to make speed, RPM, pedal, brake, steering, wheel-speed, torque, or gear bytes move clearly."]],
