...
const buildPlainEnglishSummaryReport = ({ data, fileId, anomalies, suspectIds, componentHealth }: { data: AnalysisResult; fileId?: string; anomalies: number; suspectIds: number; componentHealth: number }) => {
  ...
  const bodyIds = systems.filter((row) => String(row.category ?? "").includes("body") || String(row.category ?? "").includes("chassis")).map((row) => renderText(row.id ?? row.key)).slice(0, 8);
  const powertrainIds = systems.filter((row) => String(row.category ?? "").includes("powertrain")).map((row) => renderText(row.id ?? row.key)).slice(0, 8);
  const diagnosticIds = systems.filter((row) => String(row.category ?? "").includes("diagnostic")).map((row) => renderText(row.id ?? row.key)).slice(0, 8);
  ...
  const sections = [
    ...
    ["What This Log Is", ["This is an awake periodic CAN capture with unresolved physical meaning, not enough evidence by itself to prove motion or idle state.", "Modules are awake and broadcasting periodic status frames.", "The log is useful for module mapping, baseline behavior, status frames, counters, timers, flags, and identifying which IDs are stable versus dynamic.", "The log is not enough for confident motion decoding because no validated speed, RPM, pedal, brake, steering, wheel-speed, torque, or gear signal was isolated."]],
